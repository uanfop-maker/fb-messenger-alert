// FB Alert Content Script v4.7.7 — port-disconnect + full try-catch guard
(function () {
  'use strict';
  ...
  // 2. try-catch in every async callback body (reactive, handles race where
  //    callback started before port disconnect was detected)
  let _dead = false;
  let _scanTimer = null;
  let _titleObserver = null;

  function killScript() {
    // Clear config to release stale references faster
    _dead = true;
  ...
  function safeSend(msg) {
    if (!isAlive()) return;
    try { chrome.runtime.sendMessage(msg).catch(() => {}); } catch (e) { killScript(); }
  }
  ...
  function reloadCfg() {
    if (!isAlive()) return;
    try { chrome.storage.sync.get(null, (d) => { _cfg = d || {}; }); } catch (e) { killScript(); }
  }
  reloadCfg();
  ...
  // ─── Telegram (fetch wrapped in try-catch) ────────────────────
