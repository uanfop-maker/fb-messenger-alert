...
(function () {
  ...
  function onTitleChange() {
    try {
      ...
      if (count > _lastTitleCount && Date.now() - _titleAlertedAt > 3000) {
        _titleAlertedAt = Date.now();
        const pageName = getPageName();
        const details = getFirstUnreadVisible();
        sendTelegram(pageName, details.sender, details.messageText);
        safeSend({ type: 'NEW_MESSAGE', senderName: details.sender, messageText: details.messageText, pageName, pageUrl: getPageUrl() });
  ...
  function getFirstUnreadVisible() {
    ...
    for (const titleEl of titleEls) {
      ...
      if (!isOutgoing) return { sender: data.sender, messageText: data.messageText };
    }
    return { sender: '（未知，請捲回頂端）', messageText: '（新訊息在畫面外，請捲動查看）' };
  }
