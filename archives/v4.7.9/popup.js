...
// ─── Custom sound ─────────────────────────────────────────────
chrome.storage.local.get(['customSoundName'], (d) => {
...
$('soundFile').addEventListener('change', (e) => {
  ...
  reader.onload = (ev) => {
    ...
    b64 = btoa(b64);
    chrome.storage.local.set({ customSoundB64: b64, customSoundName: file.name }, () => {
...
$('clearSound').addEventListener('click', () => {
  chrome.storage.local.remove(['customSoundB64', 'customSoundName'], () => {
    $('soundName').textContent = '預設音效（嗶嗶聲）';
